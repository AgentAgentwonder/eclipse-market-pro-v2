      ...
      try {
        ...
        const unlistenOrderUpdate = await listen<Order>('order_update', event => {
          ...
          if (order.status === 'filled' && order.walletAddress) {
            walletStore.getState().fetchBalances(order.walletAddress, true).catch(err => {
        ...
        const unlistenTransactionUpdate = await listen<TransactionUpdateEvent>(
          ...
          event => {
            ...
            if (activeAccount && (tx.from === activeAccount.publicKey || tx.to === activeAccount.publicKey)) {
              walletStore.getState().fetchBalances(activeAccount.publicKey, true).catch(err => {
